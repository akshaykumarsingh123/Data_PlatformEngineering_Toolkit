...
/****** Object:  StoredProcedure [dbo].[SQL_ErrorLog_Alert_Notify]    Script Date: 4/11/2025 10:39:37 AM ******/
SET ANSI_NULLS ON
GO
...
DECLARE @MSGBODY  VARCHAR(8000)  
 ...
 BEGIN  
  ...
  INSERT INTO #ERRTBL  
  EXEC('SELECT [Text] FROM ' + @LKDSVRNAME + '.SQL_ErrorLog')  
  ...
  BEGIN  
    ...
    @profile_name = 'DBA_ALERT', -- Modify the profile name  
    @recipients = 'dba_alerts@sunwing.ca',-- Modify the email  
    @body = @MSGBODY,   
    @subject = @SUBJECT ;   
  FETCH NEXT FROM CURSOR1  
  INTO @MSGBODY  
  ...
  CLOSE CURSOR1  
  DEALLOCATE CURSOR1   
  EXEC ('DELETE FROM ' + @LKDSVRNAME + '.SQL_ErrorLog')   
 ...
 BEGIN  
  ...
  CLOSE CURSOR1  
  DEALLOCATE CURSOR1  
  DELETE FROM SQL_ErrorLog   -- Modify the database nameSQL_ErrorLog   -- Modify the database name  
 END  
END  
GO
